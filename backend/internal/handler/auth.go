...
func RegisterAuthRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	r.GET("/auth/session", func(c *gin.Context) {
		...
		ok(c, gin.H{"user": publicUser, "authMode": svc.AuthMode(), "logicalModels": logicalModels, "runtimeLimits": limits, "drawingEngine": drawingEngine, "features": features})
	})
...
func isReservedAPIPathPrefix(value string) bool {
	...
	case "admin", "ai", "announcements", "assets", "auth", "canvas-projects", "channels", "desktop", "diagnostics", "features", "files", "model-catalog", "models", "oauth", "plugins", "projects", "public", "resources", "sessions", "settings", "skills", "style-profiles", "tasks", "user-data", "voice-profiles", "wallet":
		return true
	default:
		return false
	}
