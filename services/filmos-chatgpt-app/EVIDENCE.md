...
- 安全：短期 Project Grant、撤销/过期/项目隔离、1MiB JSON、loopback/Origin、审计 `event_id/result_size`、路径/密钥清洗、Prompt Injection警告、代理媒体限制、断线失败；Python Preview 子进程只接收固定的 `PYTHONPATH`、Signing Secret 与 locale 四项环境值，不继承父进程环境。
...
| `npm --prefix services/filmos-chatgpt-app test` | 22 passed |
