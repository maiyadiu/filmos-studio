...
SOURCE_CHECKS = tuple(check for check in RC_LOCAL_CHECKS if check.check_id in SOURCE_CHECK_IDS) + (
    Check(
        ...
        ("go", "test", "./internal/service", "./internal/handler", "-run", "TestScriptRevision|TestScriptBatch|TestProjectScriptHTTPAuthConflictAndReadback", "-count=1"),
        ROOT / "backend",
    ...
    Check(
        ...
        ("bun", "test", "test/project-script-tools.test.ts", "test/project-script-creation.test.ts", "test/script-creation-launch.test.ts"),
        ROOT / "web",
        ("03-project-ui", "08-agent", "13-qa"),
    ),
    Check(
    ...
    *(check for check in RC_LOCAL_CHECKS if check.check_id == "production-generation-composition"),
    Check(
        ...
        ("node_modules/.bin/tsx", "--test", "test/agent-tool-broker.test.ts", "test/agent-session-manager.test.ts",
         ...
         "test/local-runtime-public-error.test.ts", "test/local-runtime-http.test.ts", "test/runtime-account.test.ts",
    ...
    Check(
        ...
        ("bun", "test", "test/auth-session.test.ts", "test/project-shot-tools.test.ts", "test/project-shot-review.test.ts",
         "test/project-chapter-storyboard.test.ts", "test/canvas-storyboard-summary.test.tsx", "test/canvas-storyboard-ports.test.ts", "test/canvas-prompts.test.ts", "test/canvas-sync-baseline.test.ts",
         ...
         "src/film/agent/agent-client.test.mjs", "src/film/agent/agent-session-recovery.test.mjs", "test/project-agent-context.test.ts", "src/film/agent/workbench-context.test.mjs",
