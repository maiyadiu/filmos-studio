...
import { FilmAgentGatewayError, type FilmCanvasObservationSource, type FilmCoreTransport } from "./gateway.js";
...
export class HttpFilmCoreTransport implements FilmCoreTransport {
    ...
    getProjectContext(hostProjectId: string, signal?: AbortSignal) {
        return this.request("GET", `/projects/${encodeURIComponent(hostProjectId)}/context`, undefined, signal);
    }

    getEntity(filmEntityId: string, signal?: AbortSignal) {
        return this.request("GET", `/entities/${encodeURIComponent(filmEntityId)}`, undefined, signal);
    }

    getAuditEvents(input: { targetId?: string; limit?: number }, signal?: AbortSignal) {
        const query = new URLSearchParams();
        if (input.targetId) query.set("targetId", input.targetId);
        if (input.limit) query.set("limit", String(input.limit));
        return this.request("GET", `/audit-events${query.size ? `?${query}` : ""}`, undefined, signal);
    }
    ...
    private async request(method: "GET" | "POST", pathname: string, body?: unknown, signal?: AbortSignal) {
        ...
        try {
            ...
            if (!response.ok) {
                const code = response.status === 409 ? "version_conflict" : `film_core_http_${response.status}`;
                throw new FilmAgentGatewayError(code, `Film Core 请求失败（HTTP ${response.status}）`);
            }
            return data;
        } catch (error) {
            if (error instanceof FilmAgentGatewayError) throw error;
            throw new FilmAgentGatewayError("film_core_unavailable", "Film Core 本地 Sidecar 不可用或响应无效");
        } finally {
            clearTimeout(timer);
...
export class CanvasAgentObservationSource implements FilmCanvasObservationSource {
    ...
    async current(signal?: AbortSignal) {
        const response = await this.fetchImpl(`${this.config.url}/api/tools`, {
            method: "POST",
            headers: { "content-type": "application/json", "x-canvas-agent-token": this.config.token },
            body: JSON.stringify({ name: "canvas_get_context", input: {} }),
            signal,
            redirect: "manual",
        });
        const envelope = await readBoundedJson(response);
        if (!response.ok || !envelope || typeof envelope !== "object" || Array.isArray(envelope)) {
            throw new FilmAgentGatewayError("canvas_context_unavailable", "无法读取当前画布上下文");
        }
        const source = envelope as Record<string, unknown>;
        const result = source.result;
        if (source.ok !== true || !result || typeof result !== "object" || Array.isArray(result)) {
            throw new FilmAgentGatewayError("canvas_context_unavailable", "当前画布上下文不可用");
        }
        const context = result as Record<string, unknown>;
        return { revision: Number(context.revision), stateHash: String(context.stateHash || "") };
    }
}
...
export function filmCoreBaseUrl(env: NodeJS.ProcessEnv = process.env) {
    return normalizeFilmCoreBaseUrl(env.FILMOS_CORE_URL || "http://127.0.0.1:17471/film");
}
...
export function normalizeFilmCoreBaseUrl(value: string) {
    ...
    if (url.protocol !== "http:" || url.hostname !== "127.0.0.1" || !url.port || url.username || url.password || url.search || url.hash) {
        throw new Error("FILMOS_CORE_URL must be an exact loopback http://127.0.0.1:<port>/film URL");
    }
...
async function readBoundedJson(response: Response): Promise<unknown> {
    ...
    if (Number.isFinite(declared) && declared > RESPONSE_LIMIT_BYTES) throw new Error("response too large");
