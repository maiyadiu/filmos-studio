...
import type {
  FilmAgentAuditRecord,
  FilmAgentAuditSink,
  FilmAgentAuditOutcome,
} from "./audit.js";
import {
...
export class FilmAgentGateway {
  ...
  async callTool(name: unknown, rawInput: unknown, signal?: AbortSignal) {
    if (!isFilmToolName(name))
      throw new FilmAgentGatewayError(
        "unknown_tool",
        `未知 Film 工具：${String(name)}`,
      );
    const input = parseFilmToolInput(name, rawInput) as JsonRecord;
    if (name === "film_project_get_context")
      return this.readProjectContext(input.host_project_id as string, signal);
    if (name === "film_entity_get")
      return this.readEntity(input.film_entity_id as string, signal);
    if (name === "film_audit_events_get")
      return this.readAuditEvents(input, signal);
    if (name === "film_command_preview") return this.preview(input, signal);
    return this.apply(input, signal);
  }

  private async readProjectContext(
    hostProjectId: string,
    signal?: AbortSignal,
  ) {
    const canvas = await this.observeCanvas(signal);
    const data = await this.options.transport.getProjectContext(
      hostProjectId,
      signal,
    );
    const receipt = this.rememberRead(
      "project",
      hostProjectId,
      null,
      hashJson(data),
    ...
    return {
      data,
      observation: publicReadReceipt(receipt),
      agent_audit: audit,
    };
  }
  ...
  private async readEntity(filmEntityId: string, signal?: AbortSignal) {
    ...
    if (identity.id !== filmEntityId)
      throw new FilmAgentGatewayError(
        "core_identity_mismatch",
        "Film Core 返回了错误的实体 ID",
      );
    const receipt = this.rememberRead(
      "entity",
      identity.id,
  ...
  private async readAuditEvents(input: JsonRecord, signal?: AbortSignal) {
    const targetId =
      typeof input.target_id === "string" ? input.target_id : undefined;
  ...
  private async preview(input: JsonRecord, signal?: AbortSignal) {
    ...
    try {
      ...
      assertPreviewResult(data, command);
      ...
      return {
        data,
        preview_receipt: receipt.id,
        expires_at: new Date(receipt.expiresAt).toISOString(),
        agent_audit: audit,
      };
    } catch (error) {
      await this.appendDeniedOrFailed(
  ...
  private async apply(input: JsonRecord, signal?: AbortSignal) {
    ...
    const humanConfirmation = input.human_confirmation as
    ...
    try {
      ...
      const coreAuditEventId = assertApplyResult(
        data,
        command,
        this.identity.actorKind,
      );
      const audit = await this.appendAudit({
        toolName: "film_command_apply",
  ...
  private requireRead(command: FilmCommand, guards: FilmWriteGuards) {
    ...
    if (!receipt || receipt.expiresAt <= this.now().getTime()) {
      throw new FilmAgentGatewayError(
        "read_required",
        "正式命令前必须重新读取 Film 上下文或实体",
      );
    }
    if (receipt.contentHash !== guards.expected_content_hash) {
      throw new FilmAgentGatewayError(
        "content_hash_mismatch",
        "expected_content_hash 与已读取事实不一致",
      );
    }
    if (
      receipt.canvas.revision !== guards.expected_canvas_revision ||
      receipt.canvas.stateHash !== guards.expected_canvas_state_hash
    ) {
      throw new FilmAgentGatewayError(
        "read_canvas_guard_mismatch",
        "写入守卫未绑定到原始读取时的画布状态",
      );
    }
    if (command.target_id) {
      if (
        receipt.kind !== "entity" ||
        receipt.key !== command.target_id ||
        receipt.version !== command.expected_version
      ) {
        throw new FilmAgentGatewayError(
          "version_guard_mismatch",
          "target_id/expected_version 与已读取实体不一致",
        );
      }
    } else {
      ...
      if (
        ...
        !hostProjectId ||
        receipt.key !== hostProjectId
      ) {
        throw new FilmAgentGatewayError(
          "create_guard_mismatch",
          "创建命令必须绑定同一 Host Project 的上下文读取且 expected_version=0",
        );
      }
    }
    return receipt;
  ...
  private requirePreview(
    ...
    if (!receipt || receipt.expiresAt <= this.now().getTime()) {
      throw new FilmAgentGatewayError(
        "preview_required",
        "正式 Apply 前必须重新执行 Film Command Preview",
      );
    }
    if (receipt.consumed)
      throw new FilmAgentGatewayError(
        "preview_consumed",
        "Preview 收据已消费，禁止重复 Apply",
      );
    if (
      receipt.commandHash !== hashJson(command) ||
      receipt.guardsHash !== hashJson(guards) ||
      receipt.readReceipt !== readReceipt
    ) {
      throw new FilmAgentGatewayError(
        "preview_mismatch",
        "Apply 命令或并发守卫与 Preview 不一致",
      );
    }
    return receipt;
  }

  private async refreshReadBoundary(
    receipt: ReadReceipt,
    guards: FilmWriteGuards,
    signal?: AbortSignal,
  ) {
    if (receipt.kind === "entity") {
      const identity = parseCoreEntityIdentity(
        await this.options.transport.getEntity(receipt.key, signal),
      );
      if (
        identity.id !== receipt.key ||
        identity.version !== receipt.version ||
        identity.contentHash !== guards.expected_content_hash
      ) {
        throw new FilmAgentGatewayError(
          "film_state_changed",
          "Film 实体已变化，请重新读取并 Preview",
        );
      }
      return;
    }
    const context = await this.options.transport.getProjectContext(
      receipt.key,
      signal,
    );
    if (hashJson(context) !== guards.expected_content_hash) {
      throw new FilmAgentGatewayError(
        "film_context_changed",
        "Film 项目上下文已变化，请重新读取并 Preview",
      );
    }
  }
  ...
  private assertPermission(
    command: FilmCommand,
    humanConfirmation: JsonRecord | undefined,
    formalApply: boolean,
  ) {
    ...
    if (this.identity.mode === "agent" && forbidden) {
      throw new FilmAgentGatewayError(
        ...
        "Agent 不能自批 Approved 或 Locked/Script Lock",
      );
    }
    if (
      this.identity.mode === "human_only" &&
    ...
    if (
      ...
      if (humanConfirmation.confirmed_by !== this.identity.actorId) {
        throw new FilmAgentGatewayError(
          "human_confirmation_identity_mismatch",
          "人工确认人必须与当前 Human Only 操作者一致",
        );
      }
      const confirmedAt = Date.parse(String(humanConfirmation.confirmed_at));
      const ageMs = this.now().getTime() - confirmedAt;
      if (
        !Number.isFinite(confirmedAt) ||
        ageMs < -60_000 ||
        ageMs > this.receiptTtlMs
      ) {
        throw new FilmAgentGatewayError(
          "human_confirmation_expired",
          "人工确认时间必须在当前 Preview 有效窗口内",
        );
      }
    }
  }
  ...
  private async observeCanvas(signal?: AbortSignal) {
    ...
    if (
      ...
      observed.revision < 0 ||
      !/^[a-f0-9]{16,64}$/.test(observed.stateHash)
    ) {
      throw new FilmAgentGatewayError(
        "invalid_canvas_observation",
        "当前画布 revision/stateHash 无效",
      );
    }
    return observed;
  }

  private async observeAndMatchCanvas(
    guards: FilmWriteGuards,
    signal?: AbortSignal,
  ) {
  ...
  private async appendAudit(input: {
    ...
    const record: FilmAgentAuditRecord = {
      ...
      host_project_id:
        input.hostProjectId ??
        (input.command ? commandHostProjectId(input.command) : null),
      ...
      expected_canvas_revision:
        input.guards?.expected_canvas_revision ??
        input.canvas?.revision ??
        null,
      expected_canvas_state_hash:
        input.guards?.expected_canvas_state_hash ??
        input.canvas?.stateHash ??
        null,
      ...
      human_confirmed_by:
        typeof input.humanConfirmation?.confirmed_by === "string"
          ? input.humanConfirmation.confirmed_by
          : null,
      human_confirmed_at:
        typeof input.humanConfirmation?.confirmed_at === "string"
          ? input.humanConfirmation.confirmed_at
          : null,
      human_confirmation_reason:
        typeof input.humanConfirmation?.reason === "string"
          ? input.humanConfirmation.reason
          : null,
...
function parseCoreEntityIdentity(value: unknown) {
  ...
  const contentHash = filmContractPrimitives.contentHashSchema.parse(
    ref.content_hash,
  );
  return { id, version, contentHash };
}

function assertPreviewResult(value: unknown, command: FilmCommand) {
  const source = asRecord(value, "Film Core preview result");
  const validTarget = source.target_id === command.target_id;
  const validChanges =
    Array.isArray(source.changes) &&
    source.changes.every((item) => typeof item === "string");
  if (
    source.mode !== "preview" ||
    source.command_type !== command.command_type ||
    !validTarget ||
    source.current_version !== command.expected_version ||
    source.resulting_version !== command.expected_version + 1 ||
    !validChanges ||
    !/^[0-9a-f]{64}$/.test(String(source.content_hash ?? ""))
  ) {
    throw new FilmAgentGatewayError(
      "invalid_preview_result",
      "Film Core Preview 未完整匹配命令、目标、版本与内容哈希",
    );
  }
}
...
function assertApplyResult(
  ...
  if (source.mode !== "applied")
    throw new FilmAgentGatewayError(
      "invalid_apply_result",
      "Film Core 未返回 applied 状态",
    );
  const entity = parseCoreEntityIdentity(source.entity);
  if (command.target_id && entity.id !== command.target_id)
    throw new FilmAgentGatewayError(
      "core_identity_mismatch",
      "Apply 返回了错误实体",
    );
  if (entity.version !== command.expected_version + 1)
    throw new FilmAgentGatewayError(
      "core_version_mismatch",
      "Apply 返回版本不符合 expected_version + 1",
    );
  ...
  if (
    targetId !== entity.id ||
    audit.actor_kind !== actorKind ||
    typeof audit.action !== "string" ||
    !audit.action ||
    !isDateTime(audit.recorded_at)
  ) {
    throw new FilmAgentGatewayError(
      "invalid_core_audit",
      "Film Core Apply 未返回完整且匹配的审计事件",
    );
  }
  return eventId;
}
...
function detectsAgentApprovalOrLock(
  commandType: string,
  payload: unknown,
): boolean {
  if (
    /(^|[._-])(approve|approved|approval|lock|locked|scriptlock|script_lock)([._-]|$)/.test(
      commandType,
    )
  )
    return true;
  return walk(payload, (key, value) => {
    ...
    if (
      (normalized === "lock_state" || normalized === "creative_stage") &&
      value === "locked"
    )
      return true;
    return (
      ["approved", "is_approved", "script_locked", "is_locked"].includes(
        normalized,
      ) && value === true
    );
  });
...
function walk(
  ...
  return Object.entries(value as JsonRecord).some(
    ([key, item]) => predicate(key, item) || walk(item, predicate),
  );
}
