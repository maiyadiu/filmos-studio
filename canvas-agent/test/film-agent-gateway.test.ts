...
import {
  ...
  normalizeFilmCoreBaseUrl,
} from "../src/film/http.js";
...
class MockTransport implements FilmCoreTransport {
  ...
  async previewCommand(
    command: typeof updateCommand & { actor_kind?: string },
  ) {
    this.previewCalls += 1;
    return {
      mode: "preview",
...
test("Film Agent gateway is disabled by default and registers an isolated tool set only when enabled", () => {
  ...
  assert.equal(
    filmAgentGatewayEnabled({ FILMOS_AGENT_GATEWAY_ENABLED: "true" }),
    true,
  );
  const names: string[] = [];
  const server = {
  ...
  const disabled = registerFilmAgentMcp(
    server as never,
  ...
  const enabled = registerFilmAgentMcp(
    server as never,
    { url: "http://127.0.0.1:17371", token: "fixture" },
...
test("read -> preview -> apply binds Film version/hash and Canvas revision/hash", async () => {
  ...
  assert.equal(
    transport.applyCalls,
    1,
    "one Preview receipt cannot dispatch twice",
  );
});
...
test("Preview rejects missing formal guards before transport", async () => {
  const { gateway, transport } = fixture();
  await assert.rejects(
    gateway.callTool("film_command_preview", { command: updateCommand }),
  );
...
test("Human Only may preview a lock but Apply requires explicit human confirmation", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof FilmAgentGatewayError &&
      error.code === "human_confirmation_identity_mismatch",
  );
  ...
  })) as { data: { mode: string }; agent_audit: Record<string, unknown> };
  assert.equal(result.data.mode, "applied");
  assert.equal(transport.applyCalls, 1);
  assert.equal(result.agent_audit.human_confirmed_by, "reviewer-fixture");
  assert.equal(result.agent_audit.human_confirmation_reason, "人工验收后锁定");
});

test("Preview rejects a mismatched Core receipt before issuing a Preview receipt", async () => {
  const { gateway, transport } = fixture();
  const guards = guardsFrom(await readEntity(gateway));
  transport.previewCommand = async (command) => ({
    mode: "preview",
    command_type: command.command_type,
    target_id: command.target_id,
    current_version: command.expected_version,
    resulting_version: command.expected_version + 2,
    content_hash: HASH_V2,
    changes: ["states"],
  });
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof FilmAgentGatewayError &&
      error.code === "invalid_preview_result",
  );
});

test("project context requires an opaque Host id", async () => {
  const { gateway } = fixture();
  await assert.rejects(
    gateway.callTool("film_project_get_context", {
...
test("Film Core HTTP adapter accepts only an exact local sidecar URL", () => {
  assert.equal(
    normalizeFilmCoreBaseUrl("http://127.0.0.1:17471/film/"),
    "http://127.0.0.1:17471/film",
  );
  assert.throws(
    () => normalizeFilmCoreBaseUrl("https://example.com/film"),
    /exact loopback/,
  );
  assert.throws(
    () => normalizeFilmCoreBaseUrl("http://localhost:17471/film"),
    /exact loopback/,
  );
  assert.throws(
    () => normalizeFilmCoreBaseUrl("http://127.0.0.1:17471/other"),
    /path must be \/film/,
  );
});
