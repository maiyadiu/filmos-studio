...
export function ScriptRevisionHistory({ projectId, unitId, revision }: { projectId: string; unitId: string; revision: number }) {
    ...
    return <>
        <Button size="small" icon={<History className="size-3.5" />} onClick={() => setOpen(true)}>修订历史</Button>
        <Modal title="剧本修订历史与对照" open={open} onCancel={() => setOpen(false)} footer={null} width={960}>
            <p className="mb-3 text-sm text-muted-foreground">章节编辑记录，原版本保留；不代表 Film Core 已批准或锁定。高亮为首末变化之间的范围。</p>
            ...
            {error ? <Alert type="error" title="修订读取失败" description={error.message} showIcon /> : after.isPending || (version > 1 && before.isPending) ? <p role="status">正在读取保存的版本……</p> :
                <div className="grid gap-3 md:grid-cols-2">
                    <RevisionText label={version > 1 ? `修订前 v${version - 1}` : "初始版本（无上一版）"} text={oldText} other={newText} />
                    <RevisionText label={`修订后 v${version}`} text={newText} other={oldText} />
                </div>}
...
function RevisionText({ label, text, other }: { label: string; text: string; other: string }) {
    let start = 0;
    ...
    return <section aria-label={label} className="min-w-0 rounded-lg border border-border p-3">
        ...
        <pre className="thin-scrollbar max-h-[55vh] overflow-auto whitespace-pre-wrap break-words font-sans text-sm leading-7">{text.slice(0, start)}<mark className="bg-status-warning/25 text-foreground">{text.slice(start, text.length - tail)}</mark>{tail ? text.slice(-tail) : ""}</pre>
    </section>;
}
