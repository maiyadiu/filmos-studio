...
import type { ProjectProductionBindings } from "@/film/generation-routing/project-production-runtime";
import {
...
export function ProjectAIGenerationSettings({ projectId, projectName }: { projectId: string; projectName: string }) {
    ...
    useEffect(() => {
        ...
        void authority.loadProjectAuthority<ProjectProductionBindings>(projectId)
            .then((stored) => {
                ...
                setConnectionId(bindings.connection.connectionId);
                ...
                setExternalProjectId(bindings.projectPolicy.externalProjectBindings[bindings.connection.engineId]?.externalProjectId || "");
                setStrictLock(Boolean(bindings.projectLock?.taskLocks && Object.keys(bindings.projectLock.taskLocks).length));
                setAllowUpload(bindings.projectPolicy.uploadPolicy.allowProviderUpload);
                setMaxTasks(bindings.grant.maxTasks);
                setMaxCostMicrounits(bindings.grant.maxTotalCost?.amountMicrounits || "0");
                setCostUnit(bindings.grant.maxTotalCost?.unit || bindings.ledger.costUnit || "unknown");
            })
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            const evidence = catalogEvidence(selectedConnection.engineId);
            ...
            const authority = new FilmCoreHttpProductionGenerationAuthority(async () => new Map());
            await authority.ensureProjectAuthority(projectId, projectName, bindings);
            setLoadState("configured");
    ...
    return (
        <section className="py-5" aria-label="项目 AI 与生成设置">
            ...
            <div className="grid gap-3 rounded-lg bg-surface-active p-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <Field label="允许的生成引擎"><Select loading={loading} value={connectionId || undefined} options={connections.map((item) => ({ value: item.connectionId, label: `${item.engineId} · ${item.status}` }))} onChange={(value) => { setConnectionId(value); setDescriptorValue(""); }} /></Field>
            ...
            {selectedConnection ? <Alert className="mt-3" type={selectedConnection.status === "ready" ? "info" : "warning"} showIcon message={`Engine Connection：${selectedConnection.status}`} description={`Catalog Evidence：${catalogEvidenceLabel(catalogEvidence(selectedConnection.engineId))}。保存只写本地 Film Core，不会发起 Provider 请求或产生费用。`} /> : null}
        </section>
    );
}
...
function catalogEvidence(engineId: string): CatalogEvidence {
    const observedAt = new Date().toISOString();
    if (engineId === "runninghub") return { source: "remote_catalog", sourceLocatorId: "runninghub-configured-workflow-catalog", observedAt };
    if (engineId === "manual_web") return { source: "manual_unverified", enteredByActorRef: "local-project-owner", observedAt };
    return { source: "runtime_discovery", runtimeVersion: "filmos-candidate-v2.4", sourceLocatorId: `${engineId}-engine-connection`, observedAt };
}
