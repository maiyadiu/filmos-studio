...
function isWritableToolCall(call: ResponseToolCall) {
    return !MODEL_API_READ_TOOL_NAMES.has(call.function.name);
}
...
async function requestOnlineAgentModel(activeProfile: CanvasAgentMode, config: AiConfig, messages: ResponseInputMessage[], toolChoice: "auto" | "required", prompt: string, onDelta: (text: string) => void, promptCacheKey?: string) {
    ...
    if (backendModelRuntimeRequired(config)) {
        const result = await runBackendToolGenerationTask({ prompt, config, messages, tools: MODEL_API_AGENT_TOOLS, toolChoice });
        if (result.content.trim()) onDelta(result.content);
        return result;
    }
    return requestToolResponse(config, messages, MODEL_API_AGENT_TOOLS, toolChoice, onDelta, { promptCacheKey });
}
