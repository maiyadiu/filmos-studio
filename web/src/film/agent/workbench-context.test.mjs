...
import { applyWorkbenchContext, buildWorkbenchContext } from "./workbench-context.ts";

describe("workbench context bridge", () => {
    test("projects explicit IDs, selection, visible nodes and asset versions", () => {
        const nodes = [
            node("node-1", 10, 20, { contentUnitId: "unit-1", sceneId: "scene-1", directorUnitId: "director-1", shotId: "shot-1", assetVersionId: "asset-version-1" }),
            node("node-2", 2500, 20, { sceneId: "scene-2" }),
        ];
        const context = buildWorkbenchContext({ projectId: "canvas-1", domainProjectId: "project-1", title: "EP17", nodes, selectedNodeIds: new Set(["node-1"]), viewport: { x: 0, y: 0, k: 1 }, viewportSize: { width: 1200, height: 720 }, filmExpectedVersion: 4 });

        expect(context).toMatchObject({ projectId: "canvas-1", domainProjectId: "project-1", contentUnitId: "unit-1", sceneId: "scene-1", directorUnitId: "director-1", shotId: "shot-1", visibleNodeIds: ["node-1"], assetVersionIds: ["asset-version-1"], filmExpectedVersion: 4 });
