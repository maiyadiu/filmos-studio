...
test("VisualLock hash is canonical across map and collection insertion order", async () => {
    ...
    const firstCharacter = { entityId: "character-1", versionId: "character-v1", contentHash: HASH_A } satisfies VersionedLockReference;
    const secondCharacter = { entityId: "character-2", versionId: "character-v4", contentHash: HASH_B } satisfies VersionedLockReference;
...
test("VisualLock diff marks only consumers of changed dependency leaves as STALE", async () => {
    const oldProp = { entityId: "prop-1", versionId: "prop-v1", contentHash: HASH_A };
    const newProp = { entityId: "prop-1", versionId: "prop-v2", contentHash: HASH_B };
    const character = { entityId: "character-1", versionId: "character-v1", contentHash: HASH_C };
    ...
    const impact = diffVisualLockSets(previous, next, [
        { entityId: "shot-prop", dependencies: ["propStateVersions:prop-1"] },
        { entityId: "shot-all-props", dependencies: ["propStateVersions"] },
        { entityId: "shot-character", dependencies: ["characterIdentityVersions:character-1"] },
        { entityId: "shot-unrelated", dependencies: ["cameraVersion"] },
    ]);

    assert.deepEqual(impact.staleEntityIds, ["shot-all-props", "shot-prop"]);
    assert.equal(impact.changedDependencies.includes("propStateVersions"), true);
    assert.equal(impact.changedDependencies.includes("propStateVersions:prop-1"), true);
    assert.equal(impact.changedDependencies.includes("characterIdentityVersions:character-1"), false);
});
