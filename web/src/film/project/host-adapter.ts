...
import { isFilmContentUnitExtension, isFilmContentUnitKind, type FilmContentUnitExtension, type FilmContentUnitKind, type FilmFormalStateAxes } from "./contracts";
...
export function projectFilmContentUnits(snapshot: HostProjectSnapshot, extensionCandidates: readonly unknown[] = []): FilmContentUnitProjection[] {
    ...
    return [...snapshot.units].sort(compareHostUnits).map((unit) => {
...
function latestExtensionsByHostUnit(projectId: string, candidates: readonly unknown[]) {
    ...
    for (const candidate of candidates) {
        ...
        if (hostProjectId !== projectId) continue;
...
function resolveProductionCanvas(links: HostProjectSnapshot["canvasUnitLinks"], canvases: HostProjectSnapshot["canvases"]): FilmCanvasNavigationTarget | null {
    ...
    const candidates = links
        ...
        .sort((left, right) => rolePriority(left.link.role) - rolePriority(right.link.role) || right.canvas.updatedAt.localeCompare(left.canvas.updatedAt) || left.canvas.id.localeCompare(right.canvas.id));
...
function compareCanvasLinkPreference(left: HostProjectSnapshot["canvasUnitLinks"][number], right: HostProjectSnapshot["canvasUnitLinks"][number]) {
    return rolePriority(left.role) - rolePriority(right.role) || right.createdAt.localeCompare(left.createdAt) || left.id.localeCompare(right.id);
}
