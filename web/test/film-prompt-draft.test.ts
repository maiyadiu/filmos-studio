...
import { FILM_PROMPT_KERNEL_FLAG, PromptDraftCompileError, compilePromptDraft, sha256Text, type PromptDraftCompilerInput } from "@/film/prompt";
...
async function validInput(): Promise<PromptDraftCompilerInput> {
    ...
    return {
        ...
        scope: {
            ...
            directorUnit: binding("44444444-4444-4444-8444-444444444444", "director_unit", "0", "unit", "host-unit-1"),
        },
        directorIrText,
        directorIrHash: await sha256Text(directorIrText),
        visualLock: {
            binding: binding("55555555-5555-4555-8555-555555555555", "visual_lock", "f", "asset_version", "host-style-version-1"),
            lockText: visualLockText,
            lockHash: await sha256Text(visualLockText),
        },
        template: {
...
describe("Film PromptDraft compiler", () => {
    it("is deterministic, normalizes asset order and binds every source hash", async () => {
        ...
        expect(first.promptDraft.director_ir_hash).toBe(firstInput.directorIrHash);
        expect(first.promptDraft.prompt_text).toContain(firstInput.visualLock.binding.contentHash);
        expect(first.promptDraft.visual_lock_hash).toBe(firstInput.visualLock.lockHash);
        expect(first.bindings.assets.map((asset) => asset.binding.contentHash)).toEqual(["c".repeat(64), "d".repeat(64)]);
        expect(first.bindings.assets.map((asset) => asset.sourceContentHash)).toEqual(["1".repeat(64), "2".repeat(64)]);
    });
    ...
    it("changes both input and prompt lineage when locked visual content changes", async () => {
        ...
        afterInput.visualLock.lockHash = await sha256Text(afterInput.visualLock.lockText);
        afterInput.visualLock.binding.contentHash = "9".repeat(64);
    ...
    it("keeps aggregate record guards separate from raw source hashes", async () => {
        const beforeInput = await validInput();
        const before = await compilePromptDraft(beforeInput);
        const afterInput = await validInput();
        afterInput.scope.directorUnit.contentHash = "8".repeat(64);
        const after = await compilePromptDraft(afterInput);

        expect(after.promptDraft.director_ir_hash).toBe(before.promptDraft.director_ir_hash);
        expect(after.bindings.director_unit.contentHash).toBe("8".repeat(64));
        expect(after.hashes.input_hash).not.toBe(before.hashes.input_hash);
        expect(after.hashes.prompt_hash).not.toBe(before.hashes.prompt_hash);
    });

    it("changes lineage when an asset source/version hash changes", async () => {
        const beforeInput = await validInput();
        const before = await compilePromptDraft(beforeInput);
        const afterInput = await validInput();
        afterInput.assets[0]!.sourceContentHash = "e".repeat(64);
        const after = await compilePromptDraft(afterInput);

        expect(after.hashes.input_hash).not.toBe(before.hashes.input_hash);
        expect(after.hashes.prompt_hash).not.toBe(before.hashes.prompt_hash);
        expect(after.bindings.assets.some((asset) => asset.sourceContentHash === "e".repeat(64))).toBe(true);
    });
