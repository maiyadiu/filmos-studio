...
- `web/src/film/providers/provider-runtime.ts`
  ...
  - Submission Package 绑定 stable UUID v4 Film ID、Host Project ID、目标 `expectedVersion`/当前 content hash、prompt/parameter/reference/input hash。
  ...
  - Manual Result Import 验证 package/content/input hash、目标 `expectedVersion`/当前 content hash，记录 provider task、receipt ID/hash、参数、人工来源、导入人和授权证据。
...
11 pass, 0 fail, 39 expect() calls
...
Web/Canvas Agent 依赖从主工作区只读临时软链复用；命令结束后两个软链均已删除。没有安装依赖或修改锁文件。

### 静态边界

- 新增运行时与测试中未检出 `fetch`、Axios、`channelRequest`、上传、积分消费调用。
- `git diff --check` 通过。
- 源码 SHA-256：`d30116c2a6ccdf3e53bf95cac09cac75dc9c4aeee5ee5230e045c9262e73afae`。
- 测试 SHA-256：`9f05e19cf8073a5519b0aa6c01bd2b6ed6c5d36341adf6f8966c93ad54486125`。
